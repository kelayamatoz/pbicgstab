...
static void gpu_bicgstab(cublasHandle_t cublasHandle, cusparseHandle_t cusparseHandle, int m, int n, int nnz,
                          ...
                          int maxit, double tol, double ttt_sv)
{
    ...
    checkCudaErrors(cusparseDcsrmv(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, n, n, nnz, &one, descra, a, ia, ja, x_j, &zero, x_j));
    checkCudaErrors(cublasDscal(cublasHandle, n, &mone, r_j, 1));
    // checkCudaErrors(cublasDaxpy(cublasHandle, n, &one, f, 1, r_j, 1));
    checkCudaErrors(cublasDaxpy(cublasHandle, n, &one, f, 1, x_j, 1));
    // Checking if I'm getting the correct r0!
    return; 
                  ...
                  float err, float eps)
{
    ...
    // Use the python functional test's x and bias settings.
    ...
    for (count = 0; count < num_iterations; count++)
    {
        ...
        gpu_bicgstab(cublasHandle, cusparseHandle, matrixM, matrixN, nnz,
                      ...
                      maxit, tol, ttt_sv); // Seems that devPtrX is the only thing I need?
